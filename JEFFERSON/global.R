...
df <- df[df$COUNTY == "JEFFERSON",]

df_stats <- df %>% group_by(PROJECT_ID) %>% summarise_at(c("Nitrate","Alkalinity","pH","Conductivity","THard","Chloride","cnt","DB_WT","Arsenic","Calcium",
...
munidf <- df %>% 
  ...
  summarise_at(c("Nitrate", "Alkalinity","Total.Hardness","Conductivity","pH","Chloride","Arsenic","Calcium","Iron","Manganese","P","Sulfate","Potassium","Magnesium","Sodium",
                 "AG_PERC","HAY_PAST","AG_HAY_PAST","ROWCROP","DAIRY_PERC","weighted.rank","DB_WT","cnt","cnt_Thard"), 
...
shape <- readOGR(dsn = "Jefferson_Municipal_LayerSource.shp", stringsAsFactors = FALSE)
...
logo <- tags$a(tags$img(src='jefferson_logo.png', height = '50', width ='100', align = 'center'))
uwsp <- tags$a(tags$img(src='uwsp.png', height = '50', width = '225', style='float:right; margin-left: 5px; margin-right: 5px'))
uwex <- tags$a(tags$img(src='uwex.png', height = '50', width = '200', style='float:right; margin-left: 5px; margin-right: 5px'))
...
soils <- tags$a(tags$img(src='soils_jefferson.png', height = '600', width = '750', align = 'center'))
wiscland <- tags$a(tags$img(src='ag_jefferson.png', height = '600', width = '750', align = 'center'))
