...
df <- read.csv("ALL_COUNTIES_DATA_TREND_20230610.csv", header = TRUE)
...
df_stats_post2019 <- df_post2019 %>% group_by(PROJECT_ID) %>% summarise_at(c("Nitrate","Alkalinity","pH","Conductivity","THard","Chloride","cnt","DB_WT"), 
                                                         list(mean_2019=mean, median_2019=median, mi_2019n=min,max_2019=max, sum_2019=sum, sd_2019=sd), na.rm = TRUE)
