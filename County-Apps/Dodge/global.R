...
#df <- read.csv("ALL_COUNTIES_DATA_MERGE_20240131.csv", header = TRUE)
df <- read.csv("ALL_COUNTIES_DATA_TRENDS_2025.csv", header = TRUE)
...
shape <- st_read(dsn = "Dodge_Municipal_LayerSource.shp", stringsAsFactors = FALSE)
...
logo <- tags$a(tags$img(src='logo.png',
                        alt = 'Logo featuring four silhouetted geese flying in front of a large orange sun with the words ‘Dodge County’ on it.
                        Tall cattails stand to the left of the sun.',
...
soils <- tags$a(tags$img(src='soils.png',
                         alt = 'Map of Dodge County, Wisconsin showing soil drainage classifications for the Well Water Sampling Project.
                         The map uses color shading to indicate drainage types, ranging from excessively drained (red) to very poorly drained (dark blue), with water areas shown in light blue.
                         Most of the county is shaded in yellow and light orange, indicating well-drained to somewhat excessively drained soils, with pockets of green and dark green showing moderately well‑drained to poorly drained areas.
                         A small outline map of Wisconsin in the bottom right highlights Dodge County. A legend on the right lists the drainage categories with corresponding colors, and a source note at the bottom credits USDA’s Natural Resources Conservation Service and the SSURGO database.',
                         height = '600', width = '750', align = 'center'))
wiscland <- tags$a(tags$img(src='wiscland.png',
                            alt = 'Map of Dodge County, Wisconsin showing agricultural landcover classifications for the Well Water Sampling Project.
                            The county is shaded in a patchwork of colors representing different crop and land uses, including potato/vegetable (dark brown), pasture (green), hay (light green), dairy rotation (yellow), cranberries (purple), continuous corn (red), and cash grain (orange).
                            Most of the map is dominated by hay, pasture, and cash‑grain areas, with scattered patches of other crop types. Water bodies appear in light blue with black outlines.
                            A legend on the right lists each agricultural category with its corresponding color, and a small map of Wisconsin in the bottom right highlights Dodge County.
                            A source note at the bottom cites Wiscland 2.0 and indicates the map was created on February 28, 2022.',
                            height = '600', width = '750', align = 'center'))
