      ...
      if (input$var == "Nitrate"){
        ...
        proxy %>%
          ...
          addCircleMarkers(data = df1, lng = df1$LONGITUDE, lat = df1$LATITUDE,
                           ...
                           popup = paste0("Well ID: ", df1$PROJECT_ID,
                                          "<br>YEAR: ", df1$YEAR,
                                          "<br>Weighted Drainage Rank: ", df1$weighted.rank,
                                          "<br>Soil Drainage Rank (500 m buffer): ", ifelse(df1$weighted.rank <= 1.5, "Very Poorly Drained",
                                                                                                      ifelse(df1$weighted.rank > 1.5 & df1$weighted.rank <= 2.5, "Poorly Drained", 
                                                                                                             ifelse(df1$weighted.rank > 2.5 & df1$weighted.rank <= 3.5, "Somewhat Poorly Drained", 
                                                                                                                    ifelse(df1$weighted.rank > 3.5 & df1$weighted.rank <= 4.5, "Moderately Well Drained",
                                                                                                                           ifelse(df1$weighted.rank > 4.5 & df1$weighted.rank <= 5.5, "Well Drained",
                                                                                                                                  ifelse(df1$weighted.rank > 5.5 & df1$weighted.rank <= 6.5, "Somewhat Excessively Drained",
                                                                                                                                         ifelse(df1$weighted.rank > 6.5, "Excessively Drained", 
                                                                                                                                                ifelse(is.na(df1$weighted.rank),"Not Available",9999))))))))),
                           stroke = T, color = "black", weight = 0.75, radius = 5,
                           fillOpacity = 0.9, fillColor = ~pal(fill.vals)) %>%
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Number of Samples: ", shape$cnt_n,
                                    "<br>", ifelse(shape$weighted.rank <= 1.5, "Very Poorly Drained",
                                                   ifelse(shape$weighted.rank > 1.5 & shape$weighted.rank <= 2.5, "Poorly Drained", 
                                                          ifelse(shape$weighted.rank > 2.5 & shape$weighted.rank <= 3.5, "Somewhat Poorly Drained", 
                                                                 ifelse(shape$weighted.rank > 3.5 & shape$weighted.rank <= 4.5, "Moderately Well Drained",
                                                                        ifelse(shape$weighted.rank > 4.5 & shape$weighted.rank <= 5.5, "Well Drained",
                                                                               ifelse(shape$weighted.rank > 5.5 & shape$weighted.rank <= 6.5, "Somewhat Excessively Drained",
                                                                                      ifelse(shape$weighted.rank > 6.5, "Excessively Drained", 
                                                                                             ifelse(is.na(shape$weighted.rank),"Not Available",9999))))))))),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
          addLegend("bottomleft", pal = pal, values = fill.vals, title = "Soil Drainage Rank (500 m buffer)", opacity = 1)    
    ...
    output$wl_DRAIN_CAT <- renderPlotly({
      ...
      if(input$wl_analyte == "Nitrate"){
        WL2 <- { 
          ...
          ggplot(df6, aes(x = factor(YEAR), y = Nitrate, fill = factor(YEAR))) +
            geom_boxplot() +
            facet_grid(.~factor(DRAIN_CAT), labeller = label_wrap_gen(10)) +
            stat_summary(fun=mean, colour="blue", geom="point",
                         shape=18, size=3) +
            #the following line is necessary to label the boxplots with the sample number. count*0-1 tricks geom_text to plotting below the xaxis.
            geom_text(stat = "count", aes(y = (..count..)*0-1, label = ..count..)) +
            geom_hline(yintercept = 10 , linetype = "dashed" , color = "red") +
            xlab("") +
            theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title = element_text(size = 11.5, face="bold", vjust = 10)) +
            ylab("") +
