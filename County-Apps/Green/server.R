        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape@data$Municipality,
                                     ...
                                     "<br>Number of Samples: ", shape@data$cnt_n,
                                    "<br>", ifelse(shape@data$weighted.rank <= 1.5, "Very Poorly Drained",
                                                   ifelse(shape@data$weighted.rank > 1.5 & shape@data$weighted.rank <= 2.5, "Poorly Drained", 
                                                          ifelse(shape@data$weighted.rank > 2.5 & shape@data$weighted.rank <= 3.5, "Somewhat Poorly Drained", 
                                                                 ifelse(shape@data$weighted.rank > 3.5 & shape@data$weighted.rank <= 4.5, "Moderately Well Drained",
                                                                        ifelse(shape@data$weighted.rank > 4.5 & shape@data$weighted.rank <= 5.5, "Well Drained",
                                                                               ifelse(shape@data$weighted.rank > 5.5 & shape@data$weighted.rank <= 6.5, "Somewhat Excessively Drained",
                                                                                      ifelse(shape@data$weighted.rank > 6.5, "Excessively Drained", 
                                                                                             ifelse(is.na(shape@data$weighted.rank),"Not Available",9999))))))))),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
          addLegend("bottomleft", pal = pal, values = fill.vals, title = "Soil Drainage Rank (500 m buffer)", opacity = 1)    
