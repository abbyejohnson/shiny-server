...
server = function(input, output, session) {
  ...
  
  {
    if(input$year == "2007") {
      df1 <- df[df$YEAR == 2007,]
      munidf <- munidf[munidf$YEAR == 2007,]
    }else if(input$year == "2016") {
      df1 <- df[df$YEAR == 2016,]
      munidf <- munidf[munidf$YEAR == 2016,]
      ...
      df1 <- df[df$YEAR == 2024,]
      munidf <- munidf[munidf$YEAR == 2024,]
    }else{
      df1 <- df[df$YEAR == 2025,]
      munidf <- munidf[munidf$YEAR == 2025,]
    }
    ...
    if (input$type == "Individual Wells") {
      ...
      shape <- left_join(shape, munidf, by = "Municipality")
      fill.vals <- shape[[input$var]]
    }
    
    #max.val <- max(abs(fill.vals), na.rm = T)
    max.val <- ifelse(is.numeric(fill.vals),max_(abs(fill.vals)),0)
    ...
    #   
    ...
    }else if(input$var == "cl_rate"){
      mbreaks <- c(-40.0, -2.5, -1.0, 0, 1.0, 2.5, 5.0, (max.val+1))
      ...
      mbreaks <- c(0, 6.5, 7.0, 7.5, 8.0, 8.5, 9.0, 9.5, 10)
      ...
      if (input$var == "Nitrate"){
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      layerId = as.vector(shape$OBJECTID),
        ...
        proxy %>%
          ...
          addCircleMarkers(data = df1, lng = df1$LONGITUDE, lat = df1$LATITUDE,
                           ...
                           stroke = T, color = "black", weight = 0.75, radius = ifelse(df1$MK_NO3_PVAL > 0.1, 3.5, (3.5 + abs(df1$rate*5))),
                           fillOpacity = 0.9, fillColor = ~pal(fill.vals)) %>%
        ...
        proxy %>%
          ...
          addCircleMarkers(data = df1, lng = df1$LONGITUDE, lat = df1$LATITUDE,
                           ...
                           popup = paste0("Well ID: ", df1$PROJECT_ID,
                                          "<br>Year: ", df1$YEAR,
                                          "<br>Sample Date: ", df1$SAMP_DATE,
                                          "<br><strong>Rate of Change (mg/L per year): </strong>", round((df1$cl_rate),1)),
                           stroke = T, color = "black", weight = 0.75, radius = ifelse(df1$MK_CL_PVAL > 0.1, 3.5, (3.5 + abs(df1$cl_rate))),
                           fillOpacity = 0.9, fillColor = ~pal(fill.vals)) %>%
          #addLegend("bottomleft", pal = pal, values = fill.vals, title = "Rate of Change (mg/L per year)", opacity = 1)
          addLegend("bottomleft", pal = pal, values = c(-2, -0.25, -0.1, 0, 0.1, 0.25, 0.50, 2), title = "Rate of Change (mg/L per year)", opacity = 1)
      ...
      if (input$var == "Nitrate"){
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     "<br><strong>Nitrate-Nitrogen Concentration (mg/L)</strong>",
                                     "<br>Average: ", round(shape$Nitrate, 1),
                                     "<br>Minimum: ", shape$Nitrate_min,
                                     "<br>Maximum: ", shape$Nitrate_max,
                                     "<br>Median: ", shape$Nitrate_median,
                                     "<br>Number of Samples: ", shape$cnt_n),
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$Alkalinity, 1),
                                     "<br>Minimum: ", shape$Alkalinity_min,
                                     "<br>Maximum: ", shape$Alkalinity_max,
                                     "<br>Median: ", shape$Alkalinity_med,
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
          addLegend("bottomleft", pal = pal, values = fill.vals, title = paste(input$var), opacity = 1)
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$Chloride,1),
                                     "<br>Minimum: ", shape$Chloride_min,
                                     "<br>Maximum: ", shape$Chloride_max,
                                     "<br>Median: ", shape$Chloride_med,
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$Conductivity,1),
                                     "<br>Minimum: ", shape$Conductivity_min,
                                     "<br>Maximum: ", shape$Conductivity_max,
                                     "<br>Median: ", shape$Conductivity_med,
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
          addLegend("bottomleft", pal = pal2, values = fill.vals, title = paste(input$var), opacity = 1)
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$pH,1),
                                     "<br>Minimum: ", shape$pH_min,
                                     "<br>Maximum: ", shape$pH_max,
                                     "<br>Median: ", shape$pH_med,
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
          addLegend("bottomleft", pal = pal, values = fill.vals, title = paste(input$var), opacity = 1)
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$DB_WT,1),
                                     "<br>Minimum: ", shape$DB_WT_min,
                                     "<br>Maximum: ", shape$DB_WT_max,
                                     "<br>Median: ", shape$DB_WT_med,
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$ROWCROP,2),
                                     "<br>Minimum: ", round(shape$ROWCROP_min,2),
                                     "<br>Maximum: ", round(shape$ROWCROP_max,2),
                                     "<br>Median: ", round(shape$ROWCROP_med,2),
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$DAIRY_PERC,2),
                                     "<br>Minimum: ", round(shape$DAIRY_PERC_min,2),
                                     "<br>Maximum: ", round(shape$DAIRY_PERC_max,2),
                                     "<br>Median: ", round(shape$DAIRY_PERC_med,2),
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$HAY_PAST,2),
                                     "<br>Minimum: ", round(shape$HAY_PAST_min,2),
                                     "<br>Maximum: ", round(shape$HAY_PAST_max,2),
                                     "<br>Median: ", round(shape$HAY_PAST_med,2),
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$AG_HAY_PAST,2),
                                     "<br>Minimum: ", round(shape$AG_HAY_PAST_min,2),
                                     "<br>Maximum: ", round(shape$AG_HAY_PAST_max,2),
                                     "<br>Median: ", round(shape$AG_HAY_PAST_med,2),
                                     "<br>Number of Samples: ", shape$cnt_n),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$weighted.rank,1),
                                     "<br>Minimum: ", round(shape$weighted.rank_min,1),
                                     "<br>Maximum: ", round(shape$weighted.rank_max,1),
                                     "<br>Median: ", round(shape$weighted.rank_med,1),
                                     "<br>Number of Samples: ", shape$cnt_n,
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                ...
                                highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%    
          addLegend("bottomleft", title = "Not applicable for Municipal View",
                    colors = c("white"),
                    labels = c(""),
                    opacity = 1)   
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      #fillColor = ~pal(fill.vals), fillOpacity = 0.9,
                      popup = paste0(shape$Municipality,
                                     "<br>Nitrate Trend not applicable for municipality display</strong>"),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%    
          addLegend("bottomleft", title = "Not applicable for Municipal View",
                    colors = c("white"),
                    labels = c(""),
        ...
        proxy %>%
          ...
          addPolygons(data = shape,
                      ...
                      popup = paste0(shape$Municipality,
                                     ...
                                     "<br>Average: ", round(shape$Total.Hardness,1),
                                     "<br>Minimum: ", shape$Total.Hardness_min,
                                     "<br>Maximum: ", shape$Total.Hardness_max,
                                     "<br>Median: ", shape$Total.Hardness_med,
                                     "<br>Number of Samples: ", (shape$cnt_n - shape$cnt_Thard_n)),
                      highlight = highlightOptions(weight = 2.5, fillOpacity = 0.9, bringToFront = T)) %>%
          addLegend("bottomleft", pal = pal1, values = fill.vals, title = paste(input$var), opacity = 1)
      }
    }
...
#County Summary statistics tab    
    ...
    # )
    # 
    # selected_col <- cty_var_map[[ input$cty_analyte ]]
    # 
    # df_local <- df
    # 
    # df_plot <- df_local %>% filter(!is.na(.data[[ selected_col ]]))
  ...
  output$well_nitrate <- renderPlotly({
    ...
    breaks <- as.numeric(c("2007","2016","2019","2020","2021","2022","2023","2024","2025"))
    w1 <- { 
  ...
  ###If this doesn't work it could be caused by multiple samples being collected in 2022
  #Nitrate
  output$trend_fig <- renderPlotly({
    
    df4 <- subset(df, df$YEAR >= 2019)
    df4 <- subset(df4, cnt_sum_2019==7)
    #df4 <- subset(df4, df$YEAR >= 2019)
    #df4 <- subset(df4, df4$YEAR >= 2019)
    ...
    t1 <- {
      ...
      if(input$trend_analyte == "Nitrate"){
        ...
        ggplot(df4, aes(x = reorder(PROJECT_ID, -THard_mean_2019), y = YEAR)) +
          geom_raster(aes(fill = Total.Hardness, 
                          text = paste('Project ID:', PROJECT_ID,
                                       '</br>Total Hardness Mean (mg/L as CaCO3):', THard_mean_2019))) +
        ...
        ggplot(df4, aes(x = reorder(PROJECT_ID, -Conductivity_mean_2019), y = YEAR)) +
          geom_raster(aes(fill = Conductivity, 
                          text = paste('Project ID:', PROJECT_ID,
                                       '</br>Conductivity Mean (umhos/cm):', Conductivity_mean_2019))) +
        ...
        ggplot(df4, aes(x = reorder(PROJECT_ID, -pH_mean_2019), y = YEAR)) +
          geom_raster(aes(fill = pH, 
                          text = paste('Project ID:', PROJECT_ID,
                                       '</br>pH Mean (standard units):', pH_mean_2019))) +
...
#Generate values to populate an annual summmary Table with 5 rows x 6 columns
     t1_r1_c1 <- min_(df4$Total.Hardness, ignore_na = TRUE)
     t1_r1_c2 <- min_(df4$Alkalinity, ignore_na = TRUE)
     ...
     t1_r2_c1 <- round(mean_(df4$Total.Hardness, ignore_na = TRUE), digits = 1)
     t1_r2_c2 <- round(mean_(df4$Alkalinity, ignore_na = TRUE), digits = 1)
     ...
     t1_r4_c1 <- max_(df4$Total.Hardness, ignore_na = TRUE)
     t1_r4_c2 <- max_(df4$Alkalinity, ignore_na = TRUE)
  ...
  output$index_fig <- renderPlotly({
    ...
    #Subsetting
    df5 <- subset(df, df$YEAR >= 2019)
    df5 <- subset(df5, cnt_sum_2019==7)
    #df5 <- subset(df, cnt_sum_2019==7)
    #df5 <- subset(df, YEAR >= 2019)
  ...
  #Agricultural land category
    output$wl_AG_CAT <- renderPlotly({
      ...
      #Subsetting
      df6 <- subset(df, df$YEAR >= 2019)
      df6 <- subset(df6, cnt_sum_2019==7)
