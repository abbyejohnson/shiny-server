...
#df <- read.csv("ALL_COUNTIES_DATA_TREND_20240319.csv", header = TRUE)
df <- read.csv("ALL_COUNTIES_DATA_TRENDS_2026.csv", header = TRUE)

df <- df[df$COUNTY == "CHIPPEWA",]
...
#Compute stats only for data post 2019
df_post2019 <- df[df$YEAR >= 2019,]
df_stats_post2019 <- df_post2019 %>% group_by(PROJECT_ID) %>% summarise_at(c("Nitrate","Alkalinity","pH","Conductivity","THard","Chloride","cnt","DB_WT"), 
                                                         list(mean_2019=mean, median_2019=median, min_2019=min,max_2019=max, sum_2019=sum, sd_2019=sd), na.rm = TRUE)


df_stats$nitrate_diff <- df_stats$Nitrate_max - df_stats$Nitrate_min
df <- left_join(df,df_stats,by = "PROJECT_ID")
df <- left_join(df,df_stats_post2019,by="PROJECT_ID")
...
shape <- st_read(dsn = "chippewa_municipal.shp", stringsAsFactors = FALSE)
counties <- readRDS("WIcounties.rds")

colnames(df)


shape$Municipality <- as.character(shape$MCD_NAME)



logo <- tags$a(tags$img(src='logo.png',
                        alt = 'Logos for Chippewa County agencies. On the left is the Chippewa County Land Conservation and Forest Management logo, featuring a landscape with forests, fields, and water, along with the text ‘Chippewa County Land Conservation and Forest Management — Stewardship for Sustainability.’
                        On the right is the Chippewa County Public Health logo, a shield-shaped emblem with a white cross-like symbol and the text ‘Chippewa County Public Health — Prevent. Promote. Protect.',
                        height = '75', width ='275', align = 'center'))
...
soils <- tags$a(tags$img(src='soils.png',
                         alt = 'Map of Chippewa County, Wisconsin, showing soil drainage classifications used in the Chippewa County Well Water Sampling Project.
                         Areas are colored by drainage category, including excessively drained, somewhat excessively drained, well drained, moderately well drained, somewhat poorly drained, poorly drained, very poorly drained, and water.
                         A legend on the right explains the color coding, and an inset map highlights the location of Chippewa County within Wisconsin. Source: Soil Survey Geographic (SSURGO) Database; map created February 28, 2022.',
                         height = '600', width = '750', align = 'center'))
wiscland <- tags$a(tags$img(src='wiscland.png',
                            alt = 'Map of Chippewa County, Wisconsin, showing agricultural land cover classifications as part of the Chippewa County Well Water Sampling Project.
                            The map displays various crop and land‑use types including potato and vegetable fields, pasture, hay, dairy rotation, cranberries, continuous corn, and cash grain.
                            Each land cover type is shown with a different color. A small inset map of Wisconsin highlights the location of Chippewa County. Source: Wiscland 2.0; map created February 28, 2022.',
                            height = '600', width = '750', align = 'center'))
