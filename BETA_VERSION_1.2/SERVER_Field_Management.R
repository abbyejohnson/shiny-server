...
output$OP_value5 <- renderUI({
  {
    ...
    # totalInputs - TotalOutputs - Total Storage Change - Total Cover Crop Residue
    LeachableN1 <- inputTotal1 - totalOutputs1 - total_change_N - totCovCropRes1
    LeachableN1 <- ifelse(LeachableN1 < 0, 0, LeachableN1)
    LeachableN1 <- as.numeric(LeachableN1)
  }
  {
    # Getting Input Value for final calulation
    legume2 <- crops %>%
    ...
    # totalInputs - TotalOutputs - Total Storage Change - Total Cover Crop Residue
    LeachableN2 <- inputTotal2 - totalOutputs2 - total_change_N - totCovCropRes2
    LeachableN2 <- ifelse(LeachableN2 < 0, 0, LeachableN2)
    LeachableN2 <- as.numeric(LeachableN2)
  }
  
  OP5 <- ifelse(input$acres1 > 0 && input$acres2 > 0, ((LeachableN1 * input$acres1) + (LeachableN2 * input$acres2)) / (input$acres1 + input$acres2), 9999)
  OP5 <- ifelse(OP5 == 9999, 'Enter acres of each scenario', round(OP5, digits = 1))
  ...
  {
    {
      ...
      # totalInputs - TotalOutputs - Total Storage Change - Total Cover Crop Residue
      LeachableN1 <- inputTotal1 - totalOutputs1 - total_change_N - totCovCropRes1
      LeachableN1 <- ifelse(LeachableN1 > 0, LeachableN1, 0)
      LeachableN1 <- as.numeric(LeachableN1)
    }
    {
      # Getting Input Value for final calulation
      ...
      # totalInputs - TotalOutputs - Total Storage Change - Total Cover Crop Residue
      LeachableN2 <- inputTotal2 - totalOutputs2 - total_change_N - totCovCropRes2
      LeachableN2 <- ifelse(LeachableN2 > 0, LeachableN2, 0)
      LeachableN2 <- as.numeric(LeachableN2)
    }
    
    totalLeachableN <- ((LeachableN1 * input$acres1) + (LeachableN2 * input$acres2)) / (input$acres1 + input$acres2)
    totalLeachableN <- as.numeric(totalLeachableN)
  }
  rechargeEstimateT <- soil %>%
    filter(Organic.Matter.Content == input$soilType & Soil.Drainage.Classification == input$drainClass) %>%
    select(Inches.Recharge)
  rechargeEstimateT <- as.numeric(rechargeEstimateT)
  
  OP6 <- ifelse(input$acres1 > 0 && input$acres2 > 0, (totalLeachableN / rechargeEstimateT / 0.226), 9999)
  OP6 <- ifelse(OP6 == 9999, 'Enter acres of each scenario', round(OP6, digits = 1))
    ...
    fluidRow(
      column(6,
             ...
             tippy::tippy_this(elementId = "info_irri",
                               tooltip = "<span style='font-size:14px;'>Enter the nitrate-N concentration of the 
      ...
      column(6,
             ...
             tippy::tippy_this(elementId = "info_irrInch",
                               tooltip = "<span style='font-size:16px;'>Enter total inches of irrigation water applied during the growing season</span>",
                               placement = "right")
      )
    )
  }else{
...
output$rhot4 <- renderRHandsontable({
  ...
  #Scenario 1 - Getting Input Value for final calulation
  if (input$irr == 1){
    irrigation <- input$irri * input$irrInch * 0.226
  } else{
    irrigation <- 0
  ...
  #total_change_N <- input$inoN + input$orgN
  excessN1 <- ifelse(LeachableN1 < 0, LeachableN1-input$orgN, input$orgN)
  total_change_N1 <- ifelse(LeachableN1 < 0, input$inoN + LeachableN1-input$orgN, input$inoN + input$orgN)
  
  #Scenario 2 - Getting Input Value for final calulation
  legume2 <- crops %>%
    filter(Crop.Type == input$crop2 & Category == input$units2) %>%
    select(Legume)
  ...
  #total_change_N <- input$inoN + input$orgN
  excessN2 <- ifelse(LeachableN2 < 0, LeachableN2-input$orgN, input$orgN)
  total_change_N2 <- ifelse(LeachableN2 < 0, input$inoN + LeachableN2-input$orgN, input$inoN + input$orgN)
  
  
  
  cop_info <- matrix(c("Scenario 1", input$inoN, excessN1, total_change_N1,
                       "Scenario 2", input$inoN, excessN2, total_change_N2), 
                     nrow = 4, ncol = 2, 
