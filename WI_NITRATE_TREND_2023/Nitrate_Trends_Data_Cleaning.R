...
#Variable arguements throughout the code. Change here so you don't waste your time looking through this. 

  #Minimum number of samples required to use the well in data.
  ...
  #Start year of data. I DONT RECCOMEND CHANGING IT FROM 1995. Data gets consistent starting in 1995
    'startyear' <- 1995
  ...
  #This is for our nitrate trend over time when looking at entire couties. 
...
#Combine tnc, otm, muni, and ntnc to create one big data frame with all samples called df
...
#Change the County name of St. Croix to Saint Croix to match future variable names
    df$County.name[df$County.name == "St. Croix"] <- "Saint Croix"
    df$County.name[df$County.name == "St Croix"] <- "Saint Croix"
    df$County.name[df$County.name == "Fond Du Lac"] <- "Fond du Lac"
  

###############################################################################################   
###############################################################################################
    
    # Unfortunately, there are some issues with the data entry. The hard code changes are here.
    # Changing county names to the correct County
    df$County.name[df$WI.unique.well.. == 'SO619'] <- "Adams"  
    df$County.name[df$WI.unique.well.. == 'PS410'] <- "Juneau"
    df$County.name[df$WI.unique.well.. == 'BF698'] <- "Door"
    df$County.name[df$WI.unique.well.. == 'BG894'] <- "Sauk"
    df$County.name[df$WI.unique.well.. == 'BO646'] <- "Ozaukee"
    df$County.name[df$WI.unique.well.. == 'EZ642'] <- "Adams"
    df$County.name[df$WI.unique.well.. == 'EZ643'] <- "Adams"
    df$County.name[df$WI.unique.well.. == 'EZ644'] <- "Adams"
    df$County.name[df$WI.unique.well.. == 'GS117'] <- "Ashland"
    df$County.name[df$WI.unique.well.. == 'JF699'] <- "Door"
    df$County.name[df$WI.unique.well.. == 'PX372'] <- "Ashland"
    df$County.name[df$WI.unique.well.. == 'SO621'] <- "Sauk"
    df$County.name[df$WI.unique.well.. == 'YT261'] <- "Oneida"
    df$County.name[df$WI.unique.well.. == 'BO645'] <- "Ozaukee"
    df$County.name[df$WI.unique.well.. == 'GS182'] <- "Ashland"
    df$County.name[df$WI.unique.well.. == 'GT424'] <- "Iron"
    df$County.name[df$WI.unique.well.. == 'GQ969'] <- "Door"
    df$County.name[df$WI.unique.well.. == 'BO464'] <- "Sauk"  
    df$County.name[df$WI.unique.well.. == 'BG252'] <- "Manitowoc"
    
    # Going to just get rid of them for now
...
#Change LOD from a chracter string to a numeric value for future purposes. 
    df$LOD <- as.numeric(df$LOD)
...
#If Non-detect, replace values with half of the most common detection level (1/2*0.1=0.05 mg/L)
    ...
    # df$Result.amount <- if_else(df$Result.qualifier.description == "NON-DETECT" && df$LOD == "", 0.5 * as.numeric(df$LOD), df$Result.amount)
...
#Takes around 10 minutes to run.
    ...
    # Time Saver for stuff
    # saveRDS(dfclean0 , "timeSaver.rds")
    # 
    # dfclean0 <- readRDS("timeSaver.rds")
    
    
    
#Getting rid of all rows with no identified county and county number.
    # dfclean0 <- dfclean0[!is.na(dfclean0$County.code), ]
...
#If k is less than 6, then we throw out that well. Not enough statistical evidence otherwise. Same as earlier.
