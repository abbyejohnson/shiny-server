define([
  ...
  var PrintDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
          ...
          return all([this._getPrintTaskInfo(), this._getLayerTemplatesInfo()])
            .then(lang.hitch(this, function(results) {
              ...
              if (!esriLang.isDefined(taksInfo) || (taksInfo && taksInfo.error)) {
				...
				//this sets the layout and format options selects
    ...
    onLayoutChange: function(newValue) {
	  ...
	  if (newValue === 'MAP_ONLY') {
        //html.setStyle(this.authorTr, 'display', 'none');
        //html.setStyle(this.copyrightTr, 'display', 'none');
        html.setStyle(this.titleTr, 'display', 'none');
		html.setStyle(this.preserveSection, 'display', 'none');
		html.setStyle(this.legendSection, 'display', 'none');
        //html.setStyle(this.legendTr, 'display', 'none');
		html.setStyle(this.mapOnlySection, 'display', '');
        //html.setStyle(this.mapOnlyFormDijit, 'display', '');
        ...
        //html.setStyle(this.copyrightTr, 'display', '');
        html.setStyle(this.titleTr, 'display', '');
		html.setStyle(this.preserveSection, 'display', '');
		html.setStyle(this.legendSection, 'display', '');
        //html.setStyle(this.legendTr, 'display', '');
		html.setStyle(this.mapOnlySection, 'display', 'none');
        //html.setStyle(this.mapOnlyFormDijit, 'display', 'none');
        this._currentTemplateInfo = {
          layoutOptions: {
            hasTitleText: true,
    ...
    _handlePrintInfo: function(rData) {
	  //this sets the options for the layout and format select boxes
      if (!rData.isGPPrint) {
    ...
    print: function() {
      if (this.printSettingsFormDijit.isValid()) {
        var form = this.printSettingsFormDijit.get('value'); //titleNode layoutDijit formatDijit
        //lang.mixin(form, this.layoutMetadataDijit.get('value')); //was author and copyright
        this.preserve = this.preserveFormDijit.get('value'); //scale extent or force
        lang.mixin(form, this.preserve);
        this.layoutForm = this.layoutFormDijit.get('value'); //legend
        var mapQualityForm = this.mapQualityFormDijit.get('value'); //dpi
        var mapOnlyForm = this.mapOnlyFormDijit.get('value'); //width height
        lang.mixin(mapOnlyForm, mapQualityForm);

		var xWidth = mapOnlyForm.width*mapQualityForm.dpi;
		var xHeight = mapOnlyForm.height*mapQualityForm.dpi;
		mapOnlyForm.width = xWidth;
		mapOnlyForm.height = xHeight;
        ...
        template.layoutOptions = {
          authorText: hasAuthorText ? this.defaultAuthor : "",
          copyrightText: hasCopyrightText ? this.defaultCopyright : "",
