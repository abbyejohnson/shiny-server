    ...
    return declare([BaseWidgetPanel, _TemplatedMixin], {
      ...
      resize: function(tmp) {
        ...
        if (tmp) {
		  ...
		  //**added offsetLeft to prevent jumping to wleft when specified in main config
		  //**the existing offsetTop is apparently ok to override getPos when resizing
		  //^^didn't see any issues with adding for any panels
		  tmp.l = this.domNode.offsetLeft;
      ...
      _getPositionInfo: function() {
		...
		//**added to open widget at w specified coordinates in main config
