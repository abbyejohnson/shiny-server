  ...
  function(declare, lang, array, html, aspect, query, on, Deferred, mouse,
    ...
    var clazz = declare([BaseWidget, PoolControllerMixin], {
      ...
      postCreate: function() {
		...
		html.setAttr(this.logoLink, 'href', this.appConfig.logoLink
          //this.appConfig.logoLink ? this.appConfig.logoLink : "#"
        );
		html.setAttr(this.logoLink, 'target', '_blank'
        );
      ...
      _createIconNode: function(iconConfig) {
		//add to NOT display the basemap widget node
		//if (iconConfig.name == "BasemapGallery") {
		//	return;
		//}
        var node, iconUrl;
      ...
      _onIconClick: function(node) {
        if (!node.config.widgets || node.config.widgets.length === 1 ||
          ...
          if (this.openedId && this.openedId === node.config.id) {
            this._switchNodeToClose(this.openedId);
            return;
		  } else if (this.openedHelp && this.openedHelp === node.config.id) {
			this._switchNodeToClose(this.openedHelp);
            return;
          } else {
            if (this.openedId && node.config.id !== '_91') {
      ...
      _switchNodeToOpen: function(id) {
		...
		//**in my app this doesn't appear to be necessary
      ...
      _showIconContent: function(iconConfig) {
        if(iconConfig.inPanel === false){
          ...
          this.panelManager.showPanel(iconConfig).then(lang.hitch(this, function(panel) {
			if (iconConfig.id === '_91') {
				this.openedHelp = iconConfig.id;
		    } else {
			    this.openedId = iconConfig.id;
			}
