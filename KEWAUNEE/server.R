...
server = function(input, output, session) {
  ...
  
  {
    if(input$year == "2004") {
      df1 <- df[df$YEAR == 2004,]
      munidf <- munidf[munidf$YEAR == 2004,]
    }else if(input$year == "2005") {
      df1 <- df[df$YEAR == 2005,]
      munidf <- munidf[munidf$YEAR == 2005,]
    }else if(input$year == "2006") {
      df1 <- df[df$YEAR == 2006,]
      munidf <- munidf[munidf$YEAR == 2006,]
    }else if(input$year == "2007") {
      df1 <- df[df$YEAR == 2007,]
      munidf <- munidf[munidf$YEAR == 2007,]
    }else if(input$year == "2008") {
      df1 <- df[df$YEAR == 2008,]
      munidf <- munidf[munidf$YEAR == 2008,]
    }else if(input$year == "2009") {
      df1 <- df[df$YEAR == 2009,]
      munidf <- munidf[munidf$YEAR == 2009,]
    }else if(input$year == "2010") {
      df1 <- df[df$YEAR == 2010,]
      munidf <- munidf[munidf$YEAR == 2010,]
    }else if(input$year == "2011") {
      df1 <- df[df$YEAR == 2011,]
      munidf <- munidf[munidf$YEAR == 2011,]
    }else if(input$year == "2012") {
      df1 <- df[df$YEAR == 2012,]
      munidf <- munidf[munidf$YEAR == 2012,]
    }else if(input$year == "2013") {
      df1 <- df[df$YEAR == 2013,]
      munidf <- munidf[munidf$YEAR == 2013,]
    }else if(input$year == "2014") {
      df1 <- df[df$YEAR == 2014,]
      munidf <- munidf[munidf$YEAR == 2014,]
    }else if(input$year == "2015") {
      df1 <- df[df$YEAR == 2015,]
      munidf <- munidf[munidf$YEAR == 2015,]
    }else if(input$year == "2016") {
      df1 <- df[df$YEAR == 2016,]
      munidf <- munidf[munidf$YEAR == 2016,]
    }else if(input$year == "2017") {
      df1 <- df[df$YEAR == 2017,]
      munidf <- munidf[munidf$YEAR == 2017,]
    }else if(input$year == "2018") {
      df1 <- df[df$YEAR == 2018,]
      munidf <- munidf[munidf$YEAR == 2018,]
      ...
      if (input$var == "Nitrate"){
        ...
        proxy %>%
          ...
          addCircleMarkers(data = df1, lng = df1$LONGITUDE, lat = df1$LATITUDE,
                           ...
                           stroke = T, color = "black", weight = 0.75, radius = ifelse(df1$NO3_PVALUE > 0.1, 3.5, (3.5 + abs(df1$rate*5))),
                           fillOpacity = 0.9, fillColor = ~pal(fill.vals)) %>%
        ...
        proxy %>%
          ...
          addCircleMarkers(data = df1, lng = df1$LONGITUDE, lat = df1$LATITUDE,
                           ...
                           stroke = T, color = "black", weight = 0.75, radius = ifelse(df1$CL_PVALUE > 0.1, 3.5, (3.5 + abs(df1$cl_rate))),
                           fillOpacity = 0.9, fillColor = ~pal(fill.vals)) %>%
          #addLegend("bottomleft", pal = pal, values = fill.vals, title = "Rate of Change (mg/L per year)", opacity = 1)
          addLegend("bottomleft", pal = pal, values = c(-2, -0.25, -0.1, 0, 0.1, 0.25, 0.50, 2), title = "Rate of Change (mg/L per year)", opacity = 1)
  ...
  output$well_hardness <- renderPlotly({
    ...
    w5 <- { 
      ...
      ggplot(df2, aes(x = YEAR, y = Total.Hardness)) +
        geom_point(size = 3, color = "black") +
        geom_line(color = "blue") +
        ylim(min(df2$Total.Hardness)-50,round(max(df2$Total.Hardness)+50,0)) +
        theme(axis.title.x = element_blank()) +
        scale_x_continuous(breaks = seq(min(df2$YEAR), max(df2$YEAR), by = 1)) +
        theme(axis.text.x = element_text(angle = 45, hjust = 1), axis.title = element_text(size = 11.5, face="bold")) +
        ylab("mg/L as CaCO3") +
