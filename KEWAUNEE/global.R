...
df <- read.csv("KEWAUNEE_wBothTrends_v2.csv", header = TRUE)
#colnames(df)[28] <- "THard"
df <- df[df$COUNTY == "KEWAUNEE",]
...
munidf <- df %>% 
  ...
  summarise_at(c("Nitrate", "Alkalinity","Total.Hardness","Conductivity","pH","Chloride","cnt"), 
...
shape <- readOGR(dsn = "Kewaunee_Municipality.shp", stringsAsFactors = FALSE)
