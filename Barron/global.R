...
df <- read.csv("BARRON_2022-2023.csv", header = TRUE)

df <- df[df$COUNTY == "BARRON",]

df_stats <- df %>% group_by(PROJECT_ID) %>% summarise_at(c("Nitrate","Alkalinity","pH","Conductivity","THard","Chloride","cnt","Arsenic","Calcium",
...
munidf <- df %>% 
  ...
  summarise_at(c("Nitrate", "Alkalinity","Total.Hardness","Conductivity","pH","Chloride","Arsenic","Calcium","Iron","Manganese","P","Sulfate","Potassium","Magnesium","Sodium",
                 "AG_PERC","HAY_PAST","AG_HAY_PAST","ROWCROP","DAIRY_PERC","weighted.rank","cnt","cnt_Thard"), 
...
munidf2$Municipality <- munidf2$TOWNSHIP

shape <- readOGR(dsn = "Barron_Municipal_LayerSource.shp", stringsAsFactors = FALSE)
...
logo <- tags$a(tags$img(src='barron_logo.png', height = '100', width ='133', align = 'center'))
...
soils <- tags$a(tags$img(src='barron_soils.png', height = '600', width = '750', align = 'center'))
wiscland <- tags$a(tags$img(src='barron_wiscland.png', height = '600', width = '750', align = 'center'))
