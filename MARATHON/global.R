...
df <- read.csv("MARATHON_ALL_DATA_V2.csv", header = TRUE)
#df <- read.csv("JEFFERSON_wWell_Construction_v3.csv", header = TRUE)

df <- df[df$COUNTY == "MARATHON",]
...
df_stats <- df %>% group_by(PROJECT_ID) %>% summarise_at(c("Nitrate","Alkalinity","pH","Conductivity","THard","Chloride","cnt","DB_WT",
                                                           "Arsenic","Calcium","Iron","Manganese","P","Sulfate","Potassium","Magnesium","Sodium"), 
...
logo <- tags$a(tags$img(src='marathon_logo.jfif', height = '60', width ='60', align = 'center'))
...
soils <- tags$a(tags$img(src='soilz.jpg', height = '463.6', width = '600', align = 'center'))
wiscland <- tags$a(tags$img(src='landcover.jpg', height = '463.6', width = '600', align = 'center'))
